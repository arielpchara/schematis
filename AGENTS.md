...
Merging a PR to `main` triggers CI. A `Tag` job reads `package.json` and pushes `v{version}` if it does not yet exist; the existing `Publish` job then runs on that tag. Agents must not create tags.
